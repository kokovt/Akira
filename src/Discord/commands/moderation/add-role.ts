...
/**
 * @const {command} ADDROLE
 * @summary Defines the `/add-role` slash command.
 * @description An administrative command that allows a user with the necessary
 * permissions to add a specified role to another member within the server.
 */
 ...
 */
  ...
  */
  run: async (APOLLYON, interaction) => {
    ...
    // Add the specified role to the target member.
    //TODO: It always says there is an error. Figure this out.
